...
export async function loadCompileShaders(shaderSource: string, features: FEATURES[]) {
  const shader = loadSingleShader(shaderSource)
  return `precision mediump float;\n#define M_PI ${Math.PI}\n${await Promise.all(
    assembleRequirements(features).map(loadSingleShader),
  ).then(it => it.join('\n'))}\n${await shader}`
}
...
export async function loadSingleShader(path: FEATURES | string): Promise<string> {
  // @ts-expect-error features map access
